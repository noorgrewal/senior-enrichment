  ...
  handleSubmit(e) {
    ...
    // editing existing student
      ...
      studentImage = (studentImage === '')
          ? this.state.student.image
          : studentImage;
      studentCampus = (studentCampus === '')
          ? this.state.student.campusId
          : studentCampus;
          ...
          <form onSubmit={this.handleSubmit}>
            ...
            <div className="form-group">
              ...
              <input type="text" className="form-control" id="studentImage"
                     defaultValue={this.formNew
                         ? ('https://s-media-cache-ak0.pinimg.com/736x/4a/e6/61/4ae66108c1dbae2597e5d52a438d5749.jpg')
                         : (studentImage)}
                     placeholder={this.formNew
                    ...
                    return (
                        campuses.id === studentCampus ? (
                            <option key={campuses.id} value={campuses.id}
                                    selected>{campuses.name}</option>)
                            : (<option key={campuses.id}
                                       value={campuses.id}>{campuses.name}</option>)
                    );
                  })
                }
              </select>
