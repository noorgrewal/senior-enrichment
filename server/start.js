...
if (process.env.NODE_ENV !== 'production') {
    app.use(require('volleyball')); // Logging middleware (non-production only)
}  

// The code below works because `.use` returns `this` which is `app`.
// So what we want to return in the `module.exports` is `app`, and we can
// on that declaration because each method invokation returns `app` after mutating based on the middleware functions
module.exports = app
    ...
    // notice the use of `_` as the first parameter above. This is a pattern for parameters that must exist,
    // but you don't use or reference (or need) in the function body that follows.
